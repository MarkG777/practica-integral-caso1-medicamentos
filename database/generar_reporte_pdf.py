...
p('Tras ejecutar el script DDL <code>esquema_estrella_postgres.sql</code> y el pipeline de carga <code>carga_dimensiones_hechos.py</code>, se verifico la creacion y poblacion de las 5 dimensiones y la tabla de hechos en el motor local PostgreSQL 17. Las siguientes capturas fueron obtenidas mediante consultas SQL en vivo al esquema <code>ecbd_caso1</code>.')
add_image('evidencia_1_conteos.png', 'Figura 1. Conteo de registros por tabla: las 6 tablas del esquema estrella estan creadas y pobladas (2,171 hechos).')
add_image('evidencia_2_estructura_hechos.png', 'Figura 2. Estructura de fct_adquisiciones: 5 llaves foraneas, 3 medidas numericas y el target binario, todas NOT NULL.')
add_image('evidencia_3_muestra_join.png', 'Figura 3. Muestra de datos poblados uniendo la tabla de hechos con sus dimensiones (Top 12 por monto total).')
